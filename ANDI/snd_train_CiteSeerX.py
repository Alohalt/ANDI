...
class SNDTrainer:
    def __init__(self, w_author=1.5, w_org=1.0, w_venue=1.0, w_title=0.33,
        ...
        self.processed_data_root = f'{DATASET_PATH}/data/gene/{args.dataset}'
    ...
    def save_pair(self,pubs, name, outlier):
        ...
        """
        dirpath = join(self.processed_data_root, self.data_type, name)
    ...
    def post_match(self, pred, outlier, pubs, name):
    ...
    def fit(self, add_sem=True, add_rel=True, if_post_match=True,
        ...
        result = {}
        if (args.task=='v3' and self.data_type in ['train', 'validation']) or args.task in ['v1', 'v2', 'x', 'dblp']:
        ...
        for n, name in enumerate(tqdm(raw_pubs)):
            # if name != "ling_huang":
            #     continue
            if self.data_type in ['train', 'validation']  or self.dataset in ['Aminer-v1', 'Aminer-v2', 'CiteSeerX', 'dblp']:
                pubs = []
                ilabel = 0
                labels = []
            ...
            if if_post_match:
                ...
                for i in cp:
                    outlier.add(i)
                pred = self.post_match(pred, outlier, pubs, name)
            ...
            if (args.task=='v3' and self.data_type in ['train', 'validation']) or args.task in ['v1', 'v2', 'x', 'dblp']:
                ...
                wf.write(f'{name},{pre},{recall},{f1},{len(pubs)},{len(set(labels))},{len(set(pred))},{len(outlier)}\n')
...
if __name__ == "__main__":
    ...
    args.task = "x"
    args.model_dir = os.path.join(args.model_dir, "cls_bert6")
    ...
    args.val = False
    args.test = True
    args.dataset = 'CiteSeerX'
    args.data_type = 'test'
    # args.feature_type = 'word2vec'


    processed_data_root = f'{DATASET_PATH}/data/gene/{args.dataset}'
    ...
    if args.val:
        logger.info(f"start validation: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}")
        args.data_type = 'validation'
        for text_weight in [0.5, 0.55, 0.65, 0.7, 0.75]:
            # for eps in [0.2, 0.21, 0.22, 0.23, 0.24, 0.25]:
            for min in [2, 3, 4, 5]:
            #     for num_walk in [4, 5, 6]:
            #         for walk_len in [10, 15, 20]:
                trainer = SNDTrainer(text_weight=text_weight, db_eps=0.22, db_min=min, num_walk=5, walk_len=15)  
                trainer.fit(add_sem=True, add_rel=True)
        # trainer.fit(add_sem=True, add_rel=False)
        # trainer.fit(add_sem=False, add_rel=True)
        logger.info(f"end validation: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}")

    if args.test:
        logger.info(f"start test: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}")
        args.data_type = 'test'
        # for text_weight in [0.8, 0.85, 0.9, 1, 1.05, 1.1, 1.15]:
        #     for eps in [0.23]:  # , 0.24, 0.25, 0.26, 0.27, 0.28, 0.29
        trainer = SNDTrainer(text_weight=0.75, db_eps=0.23, db_min=4, num_walk=5, walk_len=15)
        trainer.fit(add_sem=True, add_rel=False)
